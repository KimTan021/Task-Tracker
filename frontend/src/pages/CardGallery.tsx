    ...
    <div className="max-w-[1600px] mx-auto pb-10 animate-fade-in-up">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
        <div>
           <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--color-primary)]/10 border border-[var(--color-outline-variant)] mb-4">
             ...
             <span className="text-[10px] font-black tracking-widest text-[var(--color-primary)] uppercase">Task Visualizer v2</span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-10">
        {tasks.map((asset, idx) => {
          ...
          return (
            <div key={asset.id} 
              className={`group bg-[var(--color-surface-container-lowest)] rounded-[2rem] overflow-hidden border border-[var(--color-outline-variant)] hover:shadow-[0_40px_80px_rgba(28,27,27,0.08)] hover:-translate-y-2 transition-all duration-500 animate-fade-in-up flex flex-col cursor-pointer ${asset.status === 'completed' ? 'opacity-70 grayscale-[0.8] hover:grayscale-0 hover:opacity-100' : ''}`}
              style={{ animationDelay: `${idx * 100}ms` }}
            >
              <div className="relative aspect-[4/3] overflow-hidden bg-[var(--color-surface-container-low)]">
                <img alt={asset.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out grayscale-[20%] group-hover:grayscale-0" src={`https://picsum.photos/seed/task_${asset.id}/800/600`} />
                ...
                <div className="absolute top-5 left-5">
                  <span className={`
                    ...
                    ${asset.status === 'completed' ? 'bg-emerald-500 text-white' : asset.status === 'review' ? 'bg-orange-500 text-white' : 'bg-slate-500 text-white'}
                  `}>
              ...
              <div className="p-5 md:p-7 flex-1 flex flex-col">
                <div className="flex items-start justify-between mb-6">
                  <div className="max-w-[70%] text-left">
                    <h3 className="font-display font-black text-lg md:text-xl text-[var(--color-on-surface)] leading-tight mb-2 group-hover:text-[var(--color-primary)] transition-colors line-clamp-2">{asset.title}</h3>
                  ...
                  <span className="text-[10px] whitespace-nowrap font-black bg-[var(--color-surface-container-low)] text-[var(--color-on-surface-variant)] px-3 py-1.5 rounded-xl border border-[var(--color-outline-variant)]/20 shadow-sm">
                ...
                <div className="mt-auto pt-6 flex items-center justify-between border-t border-[var(--color-outline-variant)]">
                  <div className="flex -space-x-2">
                    <div className="w-8 h-8 rounded-full border-2 border-[var(--color-surface-container-lowest)] overflow-hidden shadow-sm group-hover:ring-2 group-hover:ring-[var(--color-primary)]/20 transition-all bg-[var(--color-primary)]/10 flex items-center justify-center text-[var(--color-primary)] text-[10px] font-bold">
                       JD
                    </div>
                  </div>
        ...
        <div className="rounded-[2rem] border-2 border-dashed border-[var(--color-outline-variant)]/20 flex flex-col items-center justify-center p-12 group cursor-pointer hover:bg-[var(--color-surface-container-low)] transition-all min-h-[300px]">
