...
export const GanttTimeline: React.FC = () => {
  ...
  // Generate 8 weeks starting from the Monday of the current week
  ...
  const weekColumns = Array.from({ length: 8 }).map((_, i) => {
  ...
  const getPixels = (dateString?: string): number | null => {
    ...
    const diffDays = (d.getTime() - startOfTimeline.getTime()) / (1000 * 60 * 60 * 24);
    return diffDays * (120 / 7); // 120px per week
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-12 gap-6 md:gap-8">
        ...
        <div className="flex flex-wrap items-center gap-3 md:gap-4 animate-fade-in-up delay-100">
           <div className="flex items-center bg-[var(--color-surface-container-low)] p-1 rounded-xl border border-[var(--color-outline-variant)]/20">
              ...
              <div className="px-3 md:px-4 text-[10px] md:text-xs font-bold text-[var(--color-on-surface)] uppercase tracking-widest">{startOfTimeline.toLocaleString('default', {month:'short'})} — {weekColumns[7].toLocaleString('default', {month:'short'})}</div>
        ...
        <div className="bg-[var(--color-surface-container-lowest)] rounded-2xl md:rounded-3xl shadow-[0_40px_80px_rgba(28,27,27,0.04)] border border-[var(--color-outline-variant)]/50 overflow-hidden flex flex-col animate-fade-in-up delay-200 min-w-[800px]">
          ...
          <div className="flex h-12 md:h-16 border-b border-[var(--color-outline-variant)]/30 bg-[var(--color-surface-container-low)]/50 backdrop-blur-md">
            ...
            >
              {weekColumns.map(date => (
                <div key={date.toISOString()} className="flex-shrink-0 w-[120px] flex items-center justify-center text-[9px] md:text-[10px] font-black tracking-widest text-[var(--color-on-surface-variant)]/60 bg-white/20">
          ...
          <div className="flex flex-col">
            {tasks.map((task) => {
              ...
              let width = getPixels(task.dueDate) ? (getPixels(task.dueDate)! - left) : 120; // fallback to 1 week if no due date
              
              if (width < 30) width = 30; // minimum width
              if (left < 0) { width += left; left = 0; } // clip to start

              const isPending = !task.startDate;

              return (
                <div key={task.id} className={`flex h-16 md:h-20 group hover:bg-[var(--color-surface-container-low)] transition-all duration-300 ${isCompleted ? 'opacity-60 grayscale hover:opacity-100 hover:grayscale-0' : ''}`}>
                  <div className="w-48 md:w-72 lg:w-96 flex-shrink-0 px-4 md:px-8 flex items-center justify-between border-r border-[var(--color-outline-variant)]/30">
                    <div className="flex items-center gap-2 md:gap-4">
                      <div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center ${isCompleted ? 'bg-slate-300 text-slate-600' : task.priority === 'High' ? 'bg-orange-500/10 text-orange-600' : 'bg-[var(--color-primary)]/10 text-[var(--color-primary)]'}`}>
                        <Icon className="w-4 h-4 md:w-5 md:h-5" />
                      </div>
                      <div className="min-w-0">
                        <span className="text-xs md:text-sm font-black text-[var(--color-on-surface)] tracking-tight truncate block">{task.title}</span>
                        <p className="text-[9px] md:text-[10px] font-bold text-[var(--color-on-surface-variant)] uppercase opacity-60">{task.priority} • {task.status.replace('_', ' ')}</p>
                      </div>
                    </div>
                  </div>
                  <div className="flex-grow relative overflow-hidden" style={{ backgroundImage: 'linear-gradient(to right, var(--color-outline-variant) 1px, transparent 1px)', backgroundSize: '120px 100%' }}>
                    {isPending ? (
                      <div className="absolute top-4 md:top-6 left-[10px] w-auto px-4 h-7 md:h-8 bg-[var(--color-surface-container-high)] text-[var(--color-on-surface-variant)] text-[9px] font-bold rounded-full border border-[var(--color-outline-variant)]/30 flex items-center shadow-sm">
                        PENDING SCHEDULING
                      </div>
                    ) : (
                      <div 
                        className={`absolute top-4 md:top-6 h-7 md:h-8 rounded-full overflow-hidden cursor-pointer group/bar border shadow-inner transition-all ${isCompleted ? 'bg-slate-100 border-slate-200' : 'bg-[var(--color-surface-container-low)] border-[var(--color-outline-variant)]/20'}`}
                        style={{ left: `${left}px`, width: `${width}px` }}
                      >
                        <div className={`h-full ${isCompleted ? 'bg-slate-400 opacity-40 rounded-full w-full' : 'bg-kinetic-animate w-[100%] rounded-full shadow-[0_0_20px_var(--color-primary)]/30'} relative`}>
            ...
            {tasks.length === 0 && (
              <div className="flex items-center justify-center h-32 md:h-48 text-[var(--color-on-surface-variant)] font-bold text-sm tracking-widest uppercase">
